namespace HoneydewCore.IO.Writers.JSON
{
    internal static class JsonReferenceSolutionModelsConstants
    {
        internal const string ProjectIdentifier = "project";
        internal const string NamespaceIdentifier = "namespace";
        internal const string ClassIdentifier = "class";
        internal const string FieldIdentifier = "field";
        internal const string MethodIdentifier = "method";
        internal const string OtherClassIdentifier = "other-class";
    }
}