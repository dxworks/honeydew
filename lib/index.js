...
exports.honeydewCommand = new Command()
  ...
  .option('-wd --working-directory', 'The working directory where honeydew will store the results folder.' +
    ` Defaults to the dxw cli honeydew installation folder (~/.dxw/honeydew/${_package.version})`,
    path.resolve(os.homedir(), '.dxw', 'honeydew', _package.version))
  .allowUnknownOption()
  .action(runHoneydew)
...
async function runHoneydew(options) {
  ...
  console.log('Workdir is ' + options.workingDir)
  await execSync(`${honeydewExePath} ${args.join(' ')}`, {cwd: options.workingDir, stdio: 'inherit'})
