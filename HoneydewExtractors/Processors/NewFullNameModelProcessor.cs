...
{
    public class NewFullNameModelProcessor : IProcessorFunction<RepositoryModel, RepositoryModel>
    {
        ...
        private readonly Dictionary<string, List<Tuple<SolutionModel, ProjectModel, IClassType>>> _classFqns = new();
        private Dictionary<string, ProjectModel> _projectModels;
        ...
        public NewFullNameModelProcessor(ILogger logger, ILogger ambiguousClassLogger, IProgressLogger progressLogger,
        ...
        public RepositoryModel Process(RepositoryModel repositoryModel)
        {
                ...
                foreach (var projectModel in solutionModel.Projects)
                {
                    foreach (var namespaceModel in projectModel.Namespaces)
                    {
                        foreach (var classModel in namespaceModel.ClassModels)
                        {
                            ...
                            AddClassModelToNamespaceGraph(classModel.Name, classModel.FilePath, namespaceModel.Name);

                            var tuple = Tuple.Create(solutionModel, projectModel, classModel);
                            ...
                            {
                                _classFqns.Add(classModel.Name, new List<Tuple<SolutionModel, ProjectModel, IClassType>>
                                {
                                    tuple
                                });
                            }
            ...
            _projectModels = repositoryModel.Solutions.SelectMany(solution => solution.Projects)
                .ToDictionary(model => model.FilePath);
        ...
        private string FindClassFullName(string className, NamespaceModel namespaceModelToStartSearchFrom,
            ...
            RepositoryModel repositoryModel, IList<IImportType> usingModels,
            ConcurrentDictionary<string, string> classLevelCache, string classFilePath = "",
            int genericParameterCount = 0)
        {
            ...
            if (_classFqns.TryGetValue($"{namespaceModelToStartSearchFrom.Name}.{classNameNonNullable}", out _))
            {
                return classLevelCache.GetOrAdd(className,
                    $"{namespaceModelToStartSearchFrom.Name}.{classNameNonNullable}{nullableString}");
            }
            ...
            if (_classNames.TryGetValue(classNameNonNullable, out var possibleClasses))
            {
                foreach (var possibleClass in possibleClasses)
                {
                    ...
                    switch (imports.Count)
                    {
                        ...
                        case 1:
                            return classLevelCache.GetOrAdd(className,
                                $"{imports[0].Name}.{className}{nullableString}");
                        default:
                        {
                    ...
                    // foreach (var projectModel in solutionModel.Projects)
                {
                    foreach (var namespaceModel in projectModel.Namespaces)
                    {
                            ...
                            // foreach (var classType in namespaceModel.ClassModels)
                        {
                            ...
                            for (var i = 0; i < classType.BaseTypes.Count; i++)
                            {
                                ...
                                AddAmbiguousNames(() =>
                                {
                                    SetEntityTypeFullName(repositoryModel, namespaceModel, projectModel,
                                        ...
                                        classType.BaseTypes[iCopy].Type, classLevelCache);
                                });
                            }

                            SetEntityAttributesFullName(repositoryModel, classType, namespaceModel, projectModel,
                                solutionModel, classType.Imports, classType.FilePath, classLevelCache);
                            ...
                            if (classType is IPropertyMembersClassType classModel)
                            {
                                ...
                                foreach (var propertyModel in classModel.Properties)
                                {
                                    AddAmbiguousNames(() =>
                                    {
                                        SetEntityTypeFullName(repositoryModel, namespaceModel, projectModel,
                                            solutionModel, classModel.Imports, classModel.FilePath,
                                            propertyModel.Type, classLevelCache);
                                    });
                                ...
                                foreach (var methodModel in classModel.Methods)
                                {
                                    if (methodModel.ReturnValue != null)
                                    {
                                        AddAmbiguousNames(() =>
                                        {
                                            SetEntityTypeFullName(repositoryModel, namespaceModel, projectModel,
                                                solutionModel, classModel.Imports, classModel.FilePath,
                                                methodModel.ReturnValue.Type, classLevelCache);
                                        });
                                ...
                                ChangeDependencyMetricFullName(repositoryModel, classModel, namespaceModel,
                                    projectModel, solutionModel, classLevelCache);
                            }
                            ...
                            currentClassCount++;
                            progressBar.Step(
                                $"{classType.Name} from {classType.FilePath} ({currentClassCount}/{_classCount}) - {(end - start) / 1000} s");
                        });
                        // }
                    }
            ...
            ConcurrentDictionary<string, string> classLevelCache)
        {
            AddAmbiguousNames(() =>
            {
                methodSkeletonType.ContainingTypeName = FindClassFullName(methodSkeletonType.ContainingTypeName,
                    namespaceModel, projectModel, solutionModel, repositoryModel, importTypes, classLevelCache,
                    classFilePath);
            });
            ...
            SetEntityAttributesFullName(repositoryModel, methodSkeletonType, namespaceModel, projectModel,
                solutionModel, importTypes, classFilePath, classLevelCache);

            SetFullNameForCalledMethods(classFilePath, methodSkeletonType.CalledMethods, namespaceModel,
                projectModel, solutionModel, repositoryModel, importTypes, classLevelCache);
            ...
            IMethodSignatureType attributeType, ConcurrentDictionary<string, string> classLevelCache)
        {
            ...
            foreach (var parameterType in attributeType.ParameterTypes)
            {
                SetParameterFullName(repositoryModel, namespaceModel, projectModel, solutionModel,
                    importTypes, filePath, parameterType, classLevelCache);
            }
        }
            ...
            IParameterType parameterType, ConcurrentDictionary<string, string> classLevelCache)
        {
            ...
            {
                SetEntityTypeFullName(repositoryModel, namespaceModel, projectModel, solutionModel, importTypes,
                    filePath, parameterType.Type, classLevelCache);
            });
            ...
            foreach (var attributeType in parameterType.Attributes)
            {
                SetAttributeFullName(repositoryModel, namespaceModel, projectModel, solutionModel,
                    importTypes, filePath, attributeType, classLevelCache);
            }
        }
            ...
            IEntityType entityType, ConcurrentDictionary<string, string> classLevelCache)
        {
            AddAmbiguousNames(() =>
            {
                SetGenericTypeFullName(repositoryModel, namespaceModel, projectModel, solutionModel, importTypes,
                    filePath, entityType.FullType, classLevelCache);
            ...
            string ReconstructFullName(GenericType genericType)
            {
                ...
                AddAmbiguousNames(() =>
                {
                    name = FindClassFullName(name,
                        namespaceModel, projectModel, solutionModel, repositoryModel,
                        importTypes, classLevelCache, filePath, genericType.ContainedTypes.Count);
                });
        ...
        private void SetGenericTypeFullName(RepositoryModel repositoryModel, NamespaceModel namespaceModel,
            ProjectModel projectModel, SolutionModel solutionModel, IList<IImportType> importTypes, string filePath,
            GenericType genericType, ConcurrentDictionary<string, string> classLevelCache)
        {
            ...
            AddAmbiguousNames(() =>
            {
                genericType.Name = FindClassFullName(genericType.Name, namespaceModel, projectModel,
                    solutionModel, repositoryModel, importTypes, classLevelCache, filePath,
            ...
            ITypeWithLocalVariables typeWithLocalVariables, ConcurrentDictionary<string, string> classLevelCache)
        {
            ...
            {
                SetEntityTypeFullName(repositoryModel, namespaceModel, projectModel, solutionModel, importTypes,
                    filePath, localVariableType.Type, classLevelCache);
            }
        }
            ...
            foreach (var calledMethod in methodModelCalledMethods)
            {
                ...
                AddAmbiguousNames(() =>
                {
                    calledMethod.ContainingTypeName = FindClassFullName(
                        ...
                        repositoryModel, importTypes, classLevelCache, classFilePath);
                ...
                foreach (var (dependencyName, appearanceCount) in dependencies)
                {
                    ...
                    try
                    {
                        var fullClassName = FindClassFullName(dependencyName, namespaceModel, projectModel,
                            solutionModel, repositoryModel, classModel.Imports, classLevelCache, classModel.FilePath);
